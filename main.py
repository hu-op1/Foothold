#!/usr/bin/env python3
"""GPU roofline benchmark suite — benchmark, fit, PD sim strategy search.

Usage:
    uv run python main.py bench                   # GPU kernel microbenchmarks
    uv run python main.py fit                     # Roofline model fitting
    uv run python main.py pd-sim                  # PD disaggregation strategy search

Single simulation with time-series output (standalone):
...
def main():
    parser = argparse.ArgumentParser(
        description="GPU Roofline Benchmark Suite — benchmark, fit, PD sim",
    ...
    # ── pd-sim ──
    pd_p = subs.add_parser("pd-sim", help="Run PD disaggregation strategy search")
    ...
    # Backward-compat: support --bench / --fit / --pd-sim flags too
    ...
    if mode == "bench":
        ...
        args.config = getattr(args, "config", "config/pd_sim.yaml")
        run_pd_sim(args)
    elif mode == "sim":
